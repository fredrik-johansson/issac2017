...
For computing the minimal polynomial of a matrix, we use
a technique known as ``spinning'' \cite{steel}.
...
We use a multimodular variant of this idea with early termination
as described in Appendix~\ref{sect:appendix}.
Heuristically, the minimal polynomial is correct as soon as the
Chinese remaindering stabilises, but a rigorous termination criterion is needed.
Our contribution is an efficient correctness test
that avoids evaluating the polynomial or taking
primes all the way up to a pessimistic worst-case bound.
...
We describe an algorithm we have implemented for efficient
...
The idea behind our multimodular version of this algorithm is to reduce the
