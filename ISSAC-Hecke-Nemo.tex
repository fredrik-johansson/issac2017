...
Nemo (\url{http://nemocas.org}) is a computer algebra package for the Julia programming language.
The eventual aim is
...
\subsection{Generic polynomial algorithms}
...
\subsection{Generic matrix algorithms}
...
\subsection{C/C++ wrappers in Nemo}
\label{sect:wrappers}


\subsubsection{Flint: arithmetic and number theory}
...
\subsubsection{Arb: arbitrary precision ball arithmetic}
...
Arb is designed to allow lazy evaluation: the user can
...
For example, in some applications, it would be more convenient to use a lazy
...
In contrast, the current model has similar performance to
ordinary arbitrary-precision floating-point arithmetic,
but with the benefit of providing error bounds.
...
\subsubsection{Antic: algebraic number fields}
