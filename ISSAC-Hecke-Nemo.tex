...
The contribution of our work is twofold. First, we demonstrate
that Julia is effective for implementing high performance computer
algebra algorithms. Second, our implementations
include a number of
improvements over the state of the art
in existing computer algebra systems.

This paper is structured as follows.
Section~\ref{sect:domains} discusses our motivation
for using Julia
and our approach to modelling mathematical domains in Julia.
Section~\ref{sect:nemo} presents basic use of Nemo with
examples and benchmarks.
In sections \ref{sect:generic} and \ref{sect:wrappers},
we discuss in more detail the generic algorithms and C/C++ wrappers
implemented in Nemo.
Finally, section~\ref{sect:hecke} discusses Hecke, a Julia
package that builds on Nemo to cover
algebraic number theory.

\section{Julia for computer algebra}
\label{sect:domains}

Julia \cite{julia} is a sophisticated, modern programming language which is designed
to be both performant and flexible. Julia was written for technical computing with a
focus on numerics. However, it has found widespread use for general purpose
applications.
Notable features include an innovative type system
(with support for parametric types and multiple dispatch),
just-in-time (JIT) compilation
supported by dynamic type inference,
and powerful metaprogramming capabilities.
In addition, Julia has an efficient native C interface, and more recently an experimental C++ interface.

Julia has a familiar imperative syntax (like Python and Matlab),
including support for operator overloading.
It includes high performance collection types (dictionaries, iterators, arrays, etc.),
automatic memory management (garbage collection) and comes with a powerful
standard library.
In addition, Julia provides an interactive console and support for Jupyter notebooks.
These features enable rapid development and make Julia
convenient as a user language.
 
One of the most important features from a mathematical point of view is Julia's parametric type system.
For example, in Julia, an array of integers has type \texttt{Array\{Int, 1\}} (the 1 refers to the dimensions
of the array).
...
Julia provides two levels of types that we make use of: abstract and concrete types.
Concrete types are as per the types that appear in languages such as Java, C or C++,
...
In this section, we present a few code examples which double as
benchmark problems.
...
$f \cdot (f+1)$ where $f = (1 + x + y + z + t)^n$. In Nemo, this
is expressed by the following Julia code:
...
Julia and C.
Flint (no asm) is essentially
the same algorithm in C for the coefficient ring $\mathbb{Z}$,
and Flint (asm) has additional assembly optimisations for the coefficient
arithmetic.
...
C libraries such as GMP/MPIR and Flint allow mutating
objects in-place, which improves performance
especially when making incremental updates
to large, nested objects.
Nemo objects are immutable, but local mutation is
...
power series (supporting both relative and absolute precision models),
and multivariate polynomials in sparse distributed format.
Most of the polynomial algorithms are standard, but there are a few notable tricks.

The generic polynomial resultant code in Nemo computes the resultant using subresultant
...
Flint's multiplication in $\mathbb{Z}[x]$
...
Antic (Algebraic Number Theory in C) is a C library, depending on Flint, for
doing efficient computations in algebraic number fields. 

Nemo makes use of Antic for speeding up number field element arithmetic. We briefly describe some
...
at random in the ideal. This representation is efficient in terms of its storage requirement in comparison to store a
...
multiplication lies in the fact that both ideals need to be given by an $S$-normal presentation for
...
Despite their character, one is often forced to study algebraic number fields in a real or complex setting, e.g. embeddings into an algebraically
...
which at the same time maintaining guaranteed results.
...
This function has an analytic continuation to the whole complex plane with a simple pol at $1$. The celebrated analytic class number formula
