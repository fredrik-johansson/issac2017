...
has automatic memory management (garbage collection) and comes with a powerful
standard library.
...
is expressed by the following Julia code (with $n = 30$):
...
multiplication algorithm for generic coefficients.
Nemo uses Flint instead of the generic
code by default over $\mathbb{Z}$, but we
have invoked the generic algorithm here to compare Julia and C.
